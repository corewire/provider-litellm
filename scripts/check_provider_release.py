#!/usr/bin/env python3
"""
Check for new releases of the LiteLLM Terraform provider
(github.com/BerriAI/terraform-provider-litellm) and act on it.
...
release?" check:
  ...
  --mode bump
      Performs the actual version bump in the *current* working tree:
        ...
        - updates the `terraform-provider-litellm` pseudo-version in `go.mod`
...
UPSTREAM_REPO = "BerriAI/terraform-provider-litellm"
GO_MODULE = "github.com/BerriAI/terraform-provider-litellm"
MAKEFILE = "Makefile"
...
def build_issue_body(current_version, new_version, release_url, release_notes):
    ...
    return f"""A new release of the [LiteLLM Terraform provider]({release_url}) is
...
- [ ] Update the `{GO_MODULE}` pseudo-version
...
def cmd_issue(args):
    ...
    title = f"chore: bump terraform-provider-litellm to v{latest_version}"
...
Bumps the LiteLLM Terraform provider from `v{current_version}` to
